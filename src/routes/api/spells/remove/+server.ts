...
export const POST: RequestHandler = async ({ url }) => {
	...
	const updatedSpellsList = spellbook.spell_ids.filter((spell) => spell !== spellId);
	...
	if (spellbook.spell_ids.includes(spellId)) {
		updateSpellbook = await prisma.spellbook.update({
			...
			data: {
				spell_ids: {
					set: updatedSpellsList
				}
			}
		});
		return json(updateSpellbook);
	} else {
		return json({ message: `Spell not in ${spellbook.spellbook_name}` }, { status: 404 });
	}
};
